...
class MassController extends Controller
{
    ...
    public function index(Request $request)
    {
        if($request->ajax())
        {
            ...
            if($request->has('status'))
            {
                ...
                $url = $this->url . 'getstatus?id=' . $this->id;
            ...
            if($request->has('all_list'))
            {
                ...
                if(Redis::hexists($this->id,'all_list'))
                {

                    //因为存储的是json字符串的数据，需要转
                    $data = json_decode(Redis::hget($this->id,'all_list'));


                } else
                {

                    $url = $this->url . 'getcontact?id=' . $this->id;
                    ...
                    if($result['success'])
                    {
                        ...
                        } else
                        {
                            ...
                            Redis::hmset(Auth::user()->email,[
                                'all_list' => $result['data'],
            ...
            if($request->has('logout'))
            {
                ...
                $url = $this->url . 'logout?id=' . $this->id;
            ...
            if($request->has('prov_id'))
            {
                ...
                $city = CityModel::select('id','city_name')
        ...
        $province = ProvinceModel::select('prov_id','prov_name')
            ->get();
    ...
    public function setMessage(Request $request)
    {
        ...
        $url = $this->url . 'setmessage?id=' .$this->id;
    ...
    public function setCondition(Request $request)
    {
        if($request->ajax())
        {
            ...
            } else
            {
                ...
                } else
                {
                    ...
                    if($input['Province'] !== '0')
                    {
                        ...
                        $province = ProvinceModel::select('prov_name')
                            ->where('prov_id',intval($input['Province']))
                            ->first();
                    ...
                    if($input['City'] !== '0')
                    {
                        ...
                        $city = CityModel::select('city_name')
                            ->where('id',intval($input['City']))
                            ->first();
            ...
            $url = $this->url . 'setcondition?id=' . $this->id;
